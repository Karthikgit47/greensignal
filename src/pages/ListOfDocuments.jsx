...
function ListOfDocuments() {
    ...
    return (
        <div>
            ...
            >
               <div  onClick={() => navigate("/dashboard/list-of-sops")}><h3>List of Documents</h3></div>
              <div style={{ fontWeight:"bold", fontSize:'20px', marginRight: "70%" }}
             >
                {"/"} <span>{sopName}</span></div>
                    ...
                    {loading ? (
                        ...
                        data.map((item, index) => (
                            <tr key={item.RecordID || index}>
                                ...
                                <td style={{ ...styles.td, width: "120px" }}>{item.TypeOfDocumentName}</td>
                                <td style={styles.td}>{item.DocumentName}</td>
                                <td style={{ ...styles.td, textAlign: "center" }}>
                                    <FaBookOpen
                                        title="Log Notes"
                                        style={{ cursor: "pointer", color: "#80b0e4" }}
                                        onClick={() => {handlelogNotes(item.RecordID, item.DocumentName)}}
                                    />
                                </td>
                            </tr>
                        ))
